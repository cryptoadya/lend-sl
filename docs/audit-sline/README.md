...
В ветке `dev` уже исправлены обе найденные проблемы мобильного меню, клиентская валидация, отправка контактной формы без JavaScript, некорректная обработка неизвестных URL и внутренняя устойчивость контактного API. Исправления проверены на Cloudflare Preview и пока не переносились на основной сайт.

Все первоочередные проблемы аудита устранены в `dev`. Следующий разумный этап — защита API от частых автоматических запросов на уровне Cloudflare и улучшение доступности, но эти изменения не блокируют текущую работу сайта.
...
Отдельно остаётся добавить rate limit/Turnstile поверх honeypot, если это потребуется по фактическому объёму спама и возможностям бесплатного плана Cloudflare.
...
6. Настроить rate limit и при необходимости Turnstile после отдельной проверки возможностей Cloudflare Free.
7. Поднять контраст полей/фокуса и добавить skip-link.
8. Затем сделать конверсионную и SEO/performance-полировку.
