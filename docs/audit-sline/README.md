...
В ветке `dev` уже исправлены обе найденные проблемы мобильного меню, клиентская валидация и отправка контактной формы без JavaScript. Исправления проверены на Cloudflare Preview и пока не переносились на основной сайт.

Перед активным продвижением остаётся один первоочередной пункт:
...
4. Добавить настоящую `404` и проверить HTTP-статусы неизвестных URL/asset-файлов.
