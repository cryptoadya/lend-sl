...
В ветке `dev` уже исправлены обе найденные проблемы мобильного меню и клиентская валидация контактной формы. Исправления проверены на Cloudflare Preview и пока не переносились на основной сайт.
...
2. безопасный сценарий контактной формы без JavaScript.
...
Форма по-прежнему содержит `novalidate` и не задаёт `method`/`action`: [ContactCTA.astro](../../src/components/ContactCTA.astro#L29). Это отдельный пункт `FORM-NOJS`. В `3cced12` клиентская логика переведена на общий `checkValidity()`/`reportValidity()`, поэтому нативный `type="email"` блокирует неверный формат до запроса. Custom validity имени, сообщения и согласия очищается на `input`/`change`: [script.js](../../public/script.js#L60).
...
3. Добавить безопасный сценарий формы без JavaScript.
