...
Перед активным продвижением я бы исправил три вещи:

1. некорректные страницы `404`, которые сейчас выглядят как обычная главная страница;
2. мобильное меню в горизонтальной ориентации и после изменения ширины окна;
3. клиентскую валидацию и аварийный сценарий контактной формы.
...
| `NAV-LANDSCAPE` | Высокий | Запланировано | Мобильное меню обрезается на низком горизонтальном экране | Последняя ссылка недоступна, при этом прокрутка страницы заблокирована | Ограничить меню через `max-height: calc(100dvh - ...)` и включить `overflow-y: auto` |
| `NAV-BREAKPOINT` | Высокий | Запланировано | Открытое мобильное меню не закрывается при переходе на desktop breakpoint | Остаётся полноэкранная подложка, а кнопка закрытия исчезает | На `matchMedia`/`resize` сбрасывать `is-open`, `nav-open` и `aria-expanded` |
...
### 9. Меню в landscape — сломанное состояние

![Обрезанное меню в горизонтальной ориентации](screenshots/09-landscape-menu.png)

При размере `667 × 320` нижний пункт меню выходит за экран. Одновременно класс `nav-open` запрещает прокрутку body, а само меню не имеет внутренней прокрутки. Пользователь не может добраться до последней ссылки.

### 10. Изменение breakpoint при открытом меню — сломанное состояние

![Мобильное меню после увеличения ширины окна](screenshots/10-breakpoint-menu.png)

Если открыть меню на мобильной ширине, а затем увеличить окно до `1000 px`, класс `is-open` остаётся. Меню продолжает работать как fixed overlay, но кнопка меню уже скрыта desktop-стилями, поэтому понятного способа закрыть его нет.

## Техническое обоснование ключевых замечаний

### Мобильное меню

В [global.css](../../src/styles/global.css#L38) класс `body.nav-open` полностью запрещает прокрутку. Открытое меню в [global.css](../../src/styles/global.css#L204) фиксировано, но не имеет ограничения по высоте и собственной прокрутки. В [script.js](../../public/script.js#L9) состояние корректно сбрасывается по ссылке и Escape, однако отсутствует обработка смены breakpoint.
...
1. Исправить мобильное меню и добавить регрессионные browser-тесты для `667 × 320` и смены breakpoint.
